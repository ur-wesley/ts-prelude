# Guide for AI assistants using `@ur-wesley/ts-prelude`

Use this file when helping a user **consume** this library in their application. For longer human-oriented examples, see [README.md](./README.md). For Cursor, use the shipped [Agent Skill](.cursor/skills/ts-prelude/SKILL.md) (copy or symlink from `node_modules/@ur-wesley/ts-prelude/.cursor/skills/ts-prelude/` into your project's `.cursor/skills/`).

## What this library is

- Functional prelude for TypeScript that **composes** [neverthrow](https://github.com/supermacro/neverthrow), [ts-pattern](https://github.com/gvergnaud/ts-pattern), and [remeda](https://remedajs.com/) — it does not replace them.
- ESM-only, `"sideEffects": false`, tree-shakeable.
- **Always use subpath imports.** The package root (`@ur-wesley/ts-prelude`) exports only `VERSION`.
...
Works with npm/pnpm/yarn as well; Bun is shown as the default in project docs.
...
**Rule:** import only the subpaths you need. Do not import from the package root for application logic.
...
See [README.md](./README.md) for full cookbook examples.
...
Typesafe env config → exhaustive match:
...
Nullable string → Option → Result → exhaustive match:
...
const message = match(parseLength(process.env.NAME))
  .with(R.ok(), (r) => {
